...
function RoutingManager({ routeRequest }: { routeRequest?: RouteRequest | null }) {
    ...
    useEffect(() => {
        ...
        const fetchRoute = async () => {
            try {
                // 1. Geocode Source (Default to Nagpur Center if 'Current Location')
                let start: [number, number] = [21.1458, 79.0882];
                if (routeRequest.source && routeRequest.source.toLowerCase() !== 'current location') {
                    const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(routeRequest.source + ', Nagpur, Maharashtra')}`);
                ...
                const resDest = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(routeRequest.destination + ', Nagpur, Maharashtra')}`);
