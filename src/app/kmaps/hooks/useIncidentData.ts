...
const NAGPUR_BOUNDS = {
    lat: { min: 21.06, max: 21.22 }, // Covers central Nagpur
    lng: { min: 78.98, max: 79.20 }  // Landlocked central bounding box
};

const LOCATIONS = [
    "Wardha Road - Ajni Square", "Kamptee Road - Kamal Square",
    "Central Avenue - Itwari", "Zero Mile - Civil Lines",
    "Amravati Road - Rachana Square", "Katol Road - Jaripatka",
    "Sitabuldi Main Road", "Wardha Road - Vayusena Nagar",
    "Hingna Road - MIDC", "Kingsway - Sadar"
];
...
export const useIncidentData = () => {
    ...
    const generateIncident = useCallback(() => {
        ...
        const newIncident: Incident = {
            ...
            type,
            location: [
                NAGPUR_BOUNDS.lat.min + Math.random() * (NAGPUR_BOUNDS.lat.max - NAGPUR_BOUNDS.lat.min),
                NAGPUR_BOUNDS.lng.min + Math.random() * (NAGPUR_BOUNDS.lng.max - NAGPUR_BOUNDS.lng.min)
            ],
