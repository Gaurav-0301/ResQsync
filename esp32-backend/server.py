"""
ESP32-CAM Detection Server for ResQsync
=========================================
...
app = FastAPI(
    title="ResQsync - ESP32 Detection Server",
...
class AlertRequest(BaseModel):
    ...
    location: str = "Video Analysis Feed"
    force: bool = False
...
async def send_alert_email(alert: AlertRequest):
    ...
    try:
        ...
        msg['Subject'] = f"ResQsync CRITICAL ALERT: {alert.type.upper()} Detected"
        ...
        <body style="font-family: Arial, sans-serif; padding: 20px;">
            <h2 style="color: #dc2626; margin-bottom: 10px;">ResQsync Emergency Incident Alert</h2>
            <p><strong>Incident Type:</strong> {alert.type.upper()}</p>
            <p><strong>Confidence:</strong> {alert.confidence * 100:.1f}%</p>
            <p><strong>Location:</strong> {alert.location}</p>
            <p><strong>Timestamp:</strong> {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
            <hr />
            <p style="color: #16a34a; font-weight: bold;">ResQsync Automated Green Corridor Protocol Initiated.</p>
        </body>
        </html>
        """
        ...
        server.quit()
        return {"status": "success", "message": "Email alert dispatched successfully"}
    except Exception as e:
        print(f"Alert recorded locally (SMTP dispatch notice: {e})")
        return {"status": "success", "message": f"Alert recorded locally"}
...
async def send_email_alert(alert: AlertRequest):
    ...
    try:
        ...
        msg['From'] = SENDER_EMAIL
        msg['To'] = RECIPIENT_EMAIL
        msg['Subject'] = f"RESQSYNC ALERT: {alert.type.upper()} Detected!"
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color:#1a1a1a;border-radius:16px;overflow:hidden;border:1px solid #333333;">
                    ...
                    <tr>
                        <td style="padding:32px;background-color:#111111;border-bottom:1px solid #333333;text-align:center;">
                            <h1 style="margin:0;color:#ffffff;font-size:24px;letter-spacing:1px;">ResQ<span style="color:#00e5ff;">sync</span></h1>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding:24px;background-color:#111111;border-top:1px solid #333333;text-align:center;">
                            <p style="margin:0;color:#444444;font-size:12px;">
                                ResQsync Automated Surveillance System<br>
...
"""
        ...
        # Plain text fallback
        text_body = f"""
RESQSYNC ALERT: {alert.type.upper()} DETECTED
...
if __name__ == "__main__":
    ...
    print("-" * 60)
    print("         RESQSYNC - ESP32 Detection Server")
    print("-" * 60)
    print("  Endpoints:")
